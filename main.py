...
def pon(call: types.CallbackQuery):
    ...
    if call.data == 'free-proxies':
        msg = bot.edit_message_text(f'Привет! Ищем бесплатные прокси для Вас...\nЧто это?\nМы ищем публичные прокси, владельцы которых сами опубликовали их на специальный сайт.\nМы парсим их и скидываем Вам 30 рабочих HTTP прокси!\nОни могут использоваться в коде, или в других целях.', call.message.chat.id, call.message.id)
        free_proxies(msg.chat.id, msg.id)
